...
const CONFIG = {
  'alert.follow': {
    tier:     'subtle',
    icon:     '❤️',
    buildMsg: p => ({ user: p.user, meta: '¡Nuevo follow!', msg: '' }),
  },
  'alert.cheer': {
    tier: p => {
      const b = p.bits || 0;
    ...
    buildMsg: p => ({ user: p.user, meta: `${p.bits} bits`, msg: p.message || '' }),
  },
  'alert.sub': {
    tier: p => p.tier === 'T3' ? 'takeover' : p.tier === 'T2' ? 'big' : 'medium',
    icon: p => p.tier === 'T3' ? '👑' : p.tier === 'T2' ? '🎖️' : '🎉',
    buildMsg: p => {
      const label = p.months > 1 ? `${p.months} meses — ${p.tier || 'T1'}` : p.tier || 'T1';
  ...
  'alert.gift_bomb': {
    tier:     'takeover',
    icon:     '💣',
    buildMsg: p => ({ user: p.user, meta: `${p.count} subs gifteados (${p.tier || 'T1'})`, msg: '' }),
  },
  'alert.raid': {
    tier: p => {
      const v = p.viewers || 0;
    ...
    icon:     '🚀',
    buildMsg: p => ({ user: p.user, meta: `Raid con ${p.viewers || 0} viewers`, msg: '' }),
  },
};

/* ── Duración según tier ────────────────────────────────────────────── */
const DURATION = { subtle: 2000, medium: 5000, big: 8000, takeover: 10000 };
...
function showAlert(event, payload) {
  ...
  const tier    = typeof conf.tier === 'function' ? conf.tier(payload) : conf.tier;
  const icon    = typeof conf.icon === 'function' ? conf.icon(payload) : conf.icon;
  const content = conf.buildMsg(payload);
  ...
  const duration = DURATION[tier] || 3000;

  if (window.gsap) {
    const fromY = tier === 'takeover' ? 0 : -60;
    gsap.fromTo(card,
      ...
      { y: 0, opacity: 1, scale: 1, duration: 0.45, ease: 'back.out(1.6)',
        onComplete: () => {
          ...
          setTimeout(() => { dismissCard(card, tier, processNext); }, duration);
        },
      }
    );
  } else {
...
const ALERT_EVENTS = new Set(['alert.follow', 'alert.cheer', 'alert.sub', 'alert.gift_bomb', 'alert.raid']);

client.on('General.Custom', msg => {
  const data = msg.data;
  if (!data || !data.event) return;

  if (ALERT_EVENTS.has(data.event)) {
